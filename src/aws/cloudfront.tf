locals {
  s3_static_origin_id = "instart-static-origin"
  s3_media_origin_id = "instart-media-origin"
}
...
resource "aws_cloudfront_origin_access_identity" "instart_oai" {
  comment = "instart-learning origin for the ${terraform.workspace} environment"
}

resource "aws_cloudfront_distribution" "instart_cloudfront_distribution" {
  # Origin for the static S3 bucket
  origin {
    domain_name = "${aws_s3_bucket.instart_static.bucket_domain_name}"
    origin_id   = "${local.s3_static_origin_id}"

    s3_origin_config {
      origin_access_identity = "${aws_cloudfront_origin_access_identity.instart_oai.cloudfront_access_identity_path}"
    }
  }

  # Origin for the media S3 bucket
  origin {
    domain_name = "${aws_s3_bucket.instart_media.bucket_domain_name}"
