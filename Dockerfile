...
COPY ./docker/files/usr/local/etc/gunicorn/instart.py /usr/local/etc/gunicorn/instart.py
...
CMD gunicorn -c /usr/local/etc/gunicorn/instart.py instart.wsgi:application
